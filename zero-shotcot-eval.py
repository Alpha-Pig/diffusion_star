...
parser.add_argument("--project_title", type=str, default="star-baseline")
...
parser.add_argument("--checkpoint", type=str, default="YOUR_CHECK")
...
run_folder = os.path.join("answers", f"{run_id}_{args.project_title}")
...
for batch_start in tqdm(range(start_question, min(args.max_idx, end_question), batch_size)):
    last_save_folder = os.path.join(run_folder, f"{args.project_title}_{args.n_ahead if not args.baseline else 1}_{args.temp}_{args.n_votes}")
    ...
    for vote_idx in range(1, args.n_votes + 1):
        ...
        save_filename = f"{folder_name}/Question:{batch_start}.txt"
